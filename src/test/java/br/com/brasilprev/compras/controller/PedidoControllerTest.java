...
public class PedidoControllerTest {
	...
	public void testController1() throws Exception {
		...
		when(pedidoService.findById(Mockito.anyLong())).thenReturn(getPedidoDto());
		
		mockMvc.perform( MockMvcRequestBuilders
			      .get("/api/pedidos/{id}", 1L)
			      .header("Authorization", "Basic dXNlcjoxMjM0NTY=")
			      .accept(MediaType.APPLICATION_JSON))
			      .andDo(print())
			      .andExpect(status().isOk())
			      .andExpect(MockMvcResultMatchers.jsonPath("$.idPedido").value(1L));
	}
